...
def parse_args():
    ...
    parser.add_argument('--bq_project', type=str, help='GCP BQ project ID')
    parser.add_argument('--bq_location', type=str, help='GCP BQ table location')
...
def main():
    ...
    for n in range(len(export_list)):
        ...
        extract_job = client.extract_table(
            ...
            job_id="export-job-{}-{}".format(str(table_ref.table_id).replace(' ', ''), round(time.time() * 1000)),
            location=args.bq_location,
            job_config=job_config)
        extract_job.result(timeout=300)
